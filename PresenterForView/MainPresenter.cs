...
namespace Presenter
{
    public class MainPresenter
    {
        ...
        public MainPresenter(IView view, IModel model, IAddHeroView addHeroView = null, StandardKernel kernel = null)
        {
            ...
            this.kernel = kernel;
            this.isConsoleMode = addHeroView == null; // Если нет формы добавления - это консоль
        ...
        private void OnChangeRepository(bool useDapper)
        {
            try
            {
                ...
                {
                    view.ShowMessage($"Репозиторий переключён: {(useDapper ? "Dapper" : "Entity Framework")}");
                }
            }
            catch (Exception ex)
            {
